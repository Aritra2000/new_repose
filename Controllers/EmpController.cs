...
namespace Db_employee_salary.Controllers
{
    public class EmpController : Controller
    {
        ...
        public async Task<IActionResult> add_emp(TblEmployee e)
        {
            try
            {
                TblEmployeeSalary s=new TblEmployeeSalary(); 
                s.Salary = e.Salary_clmn;
        ...
        public IActionResult Emp_Details(int id)
        {
            ...
            var employee = _db.TblEmployees.Join(_db.TblEmployeeSalaries,
                ...
                (t1, t2) => new TblEmployee
                {
                    ...
                    Ph = t1.Ph,
                    Salary_clmn = t2.Salary
                }).Where(a => a.EId == id).FirstOrDefault();
            return View(employee);
        }
        ...
        public async Task<IActionResult> Edit(TblEmployee e)
        {
            try
            {
                var updt = await _db.TblEmployeeSalaries.Where(a => a.Id == e.EId).FirstOrDefaultAsync();

                updt.Salary = e.Salary_clmn;

                _db.TblEmployeeSalaries.Entry(updt).State = EntityState.Modified;
        ...
        [HttpGet]
        public IActionResult Edit_sal(int id)
        {
            var employee = _db.TblEmployeeSalaries.Where(a => a.Id == id).FirstOrDefault();
            return View(employee);
        }
        [HttpPost]
        public async Task<IActionResult> Edit_sal(TblEmployeeSalary es)
        {
            try
            {
                _db.Entry(es).State = EntityState.Modified;
                await _db.SaveChangesAsync();
            }
            catch (Exception)
            {
